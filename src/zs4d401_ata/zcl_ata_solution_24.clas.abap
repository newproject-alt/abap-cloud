CLASS zcl_ata_solution_24 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_ATA_SOLUTION_24 IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    CONSTANTS c_carrier_id TYPE /dmo/carrier_id VALUE 'UA'.             "Cambio - LH por XX - UA
...
*      CATCH cx_abap_invalid_value.
      CATCH cx_abap_invalid_value into data(exc_inv).                               "------Cambio realisado - aquí guarda la essepsion
*        out->write( | Carrier { c_carrier_id } does not exist | ).                 "poh nah comentamos esto y...
        out->write( exc_inv->get_text( ) ).                                         "imprimimos la excepcion,

*      CATCH cx_abap_auth_check_exception.                                                  "vamos con este catch(hup)
      CATCH cx_abap_auth_check_exception INTO DATA(exc_auth).                               "Variable nueva creá
*        *        out->write( | No authorization to display carrier { c_carrier_id } | ).
        out->write( exc_auth->get_text( ) ).
                                                                                            "comprobamos - activamos y ejecutamos

                                "SEVILLA descanso nuevo, relajaos - a 13.45 nos vemos
    ENDTRY.
